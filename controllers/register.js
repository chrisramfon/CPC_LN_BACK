...
const mysqlconnection = require('../controllers/mysqlconnection')
...
register.reg = async(req, res)=>{
    ...
    try {
        const queryE = util.promisify(conn.conf.query).bind(mysqlconnection.conf);
        ...
        const queryU = util.promisify(conn.conf.query).bind(mysqlconnection.conf);
